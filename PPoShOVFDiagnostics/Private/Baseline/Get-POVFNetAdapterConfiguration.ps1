  ...
  process{
    ...
    if($netAdapters) {
      foreach ($interface in $netAdapters){
        ...
        $netIPInterface = Get-POVFNetIPConfiguration -PSSession $POVFPSSession -InterfaceAlias $interface.Name
        if($netIPInterface) {
          ...
          $interfaceProperties.IPConfiguration = $interfaceIPConfiguration
        }
        if($NetAdapterVMQ) {
